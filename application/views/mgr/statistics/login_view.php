                    ...
                    <td>
                        <div class="col-xs-3">
                            <select name="s_field" id="s_option" class="form-control input-sm">
                                ...
                                <option value="login_ip"<?php if($s_field == "login_ip") echo " selected"; ?>>IP</option>
                            </select>
                        </div>
                        <div class="col-xs-4">
                    ...
                    <table id="member-list" class="table table-bordered table-hover">
                        ...
                        <thead>
                            <tr calss="text-center">
                                ...
                                <th class="text-center">사용자환경</th>
                                <th class="text-center">로그인날짜</th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php
                            if ($historyList) {
                                foreach($historyList as $i => $list){
                                    $no = $total_count - ($page_rows * ($page - 1)) - $i;
                            ?>
                            <tr>
                                <td class="text-center"><?=$no?></td>
                                <td class="text-center"><?=$list->member_id?></td>
                                <td class="text-center"><?=$list->login_device?></a></td>
                                <td class="text-center"><?=$list->login_ip?></td>
                                <td><?=$list->login_agent?></td>
                                <td class="text-center"><?=date('Y-m-d H:i', strtotime($list->login_date))?></td>
                            </tr>
                            <?php
                                }
                            } else {
