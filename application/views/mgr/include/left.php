<section class="sidebar" style="height: auto;">
	...
	<div class="pull-left info">
	  <p>Hello, <?=$this->session->userdata('AName')?></p>
...
?>
    <li class="treeview <?php if ($list->main_code == substr($current_menu_code, 0, 2)) { ?> active<?php } ?>">
      ...
      <ul class="treeview-menu">
        <li <?php if ($list->sub_code == $current_menu_code) { ?> class="active"<?php } ?>><a href="<?=$list->sub_url?>"><i class="fa <?php if ($list->sub_code == $current_menu_code) {echo "fa-check-circle-o";}else { echo "fa-circle-o";} ?>"></i> <?=$list->sub_name?></a></li>
<?php
                } else {
?>
        <li <?php if ($list->sub_code == $current_menu_code) { ?> class="active"<?php } ?>><a href="<?=$list->sub_url?>"><i class="fa <?php if ($list->sub_code == $current_menu_code) {echo "fa-check-circle-o";}else { echo "fa-circle-o";} ?>"></i> <?=$list->sub_name?></a></li>
