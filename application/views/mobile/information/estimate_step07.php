<!--: Start #contents -->
<div class="contents information information-detail">
    <div class="form_ok">
        <h2>문의가 완료되었습니다.</h2>
        <img src="/mo/image/information/goods.svg" class="icon">
        <p>
            신속하게 검토 후 입력하신 연락처로<br>
            연락드리도록 하겠습니다. <br>
            문의해주셔서 감사합니다.
        </p>
    </div> 

    <div class="form_btn">
        <button class="btn btn-primary btn-w100" onclick="javascript:location.href='/'">홈으로 가기</button>
    </div>

</div>

<div class="inquiry">
    <p>편하신 경로로 문의주시면 신속하게 답변드리겠습니다.</p>
    <h2>궁금한점이 있으신가요?</h2>
    <h3>070-4352-4735</h3>
    <div class="btn_are">
        <button class="btn btn-primary" onclick="javascript:location.href='/customer/inquiry_write';">1:1 문의하기</button>
        <button class="btn btn-primary">카카오톡 문의</button>
    </div>
</div>

    <!--: End #contents -->
    
  <script>
	  ;($ => {
		  $.depth1Index = 2
		  $.depth2Index = 1
		
		  $(function () {
			
		  })
	  })(window.jQuery)
  </script>    